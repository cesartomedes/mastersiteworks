    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center">
          ...
          <nav
            className={`hidden md:flex space-x-8 mr-12 transition-all duration-300 ${textColor}`}
          >
            <Link
              to="/"
              className={`
        ...
        {isMenuOpen && (
          <div className="md:hidden mt-4 bg-[#314528]/90 backdrop-blur-md rounded-lg p-4">
            <nav className="flex flex-col space-y-3 text-white">
              <Link
                to="/"
                className={`text-lg transition duration-300 py-2 ${
                  isActive("/") && "text-white/80 font-semibold"
                }`}
                onClick={toggleMenu}
              >
                Home
              ...
              <Link
                to="/gallery"
                className={`text-lg transition duration-300 py-2 ${
                  isActive("/gallery") && "text-white/80 font-semibold"
                }`}
                onClick={toggleMenu}
              >
                Gallery
              ...
              <Link
                to="/about"
                className={`text-lg transition duration-300 py-2 ${
                  isActive("/about") && "text-white/80 font-semibold"
                }`}
                onClick={toggleMenu}
              >
                About Us
              ...
              <Link
                to="/contact"
                className={`text-lg transition duration-300 py-2 ${
                  isActive("/contact") && "text-white/80 font-semibold"
                }`}
                onClick={toggleMenu}
              >
                Contact
